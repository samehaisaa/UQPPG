...
if __name__ == "__main__":
    ...
    # This allows each model trainer to add its own arguments
    for trainer_name in dir(trainer):
        if hasattr(getattr(trainer, trainer_name), 'add_trainer_args'):
            trainer_class = getattr(getattr(trainer, trainer_name), trainer_name)
            if hasattr(trainer_class, 'add_trainer_args'):
                parser = trainer_class.add_trainer_args(parser)
